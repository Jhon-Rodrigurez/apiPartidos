...
@RestController
@RequestMapping("/partido")//endpoint
public class PartidoController {
    
    @Autowired /*se utiliza para realizar la inyección de dependencias de forma automática. método "setter" o "getter" en una clase */
    ...
    @GetMapping /*mapear una solicitud HTTP GET */
    public List<PartidoDataRestModel> leerPartidos() {
        ...
        for (PartidoDto partidoDto : partidoDtoList) { /*foreach para hacer un recorrido en la clase partidodto */
    ...
    @GetMapping(path = "/{id}") /*se está configurando un método dentro de un controlador para que maneje solicitudes GET a una ruta que contiene un parámetro
                                dinámico llamado "id". El valor de "id" será proporcionado en la URL y se pasará como un parámetro al método. */
    
    public PartidoDataRestModel detallePartido(@PathVariable String id) { /*se utiliza para mapear una variable de una URL a un parámetro en un método de controlador */
    ...
    @PostMapping
    public PartidoDataRestModel crearPartido(@RequestBody PartidoCrearRequestModel partidoCrearRequestModel) { /*se usa el requestbody para recibir y vincular automáticamente los datos enviados
